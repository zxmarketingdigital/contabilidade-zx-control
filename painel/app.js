...
(() => {
  ...
  async function login(email, senha) {
    if (ZX.mode === "demo") {
      token = ZX.token;
      return { email: email || "demo@escritorio.local" };
    }
  ...
  let tabAtual = "agentes";

  function startApp(email) {
    $("login").classList.add("hidden");
    $("app").classList.remove("hidden");
    $("userbox").textContent = email;
    $("tabs").innerHTML = TABS.map(([id, lbl]) => `<button data-tab="${id}">${lbl}</button>`).join("");
  ...
  function render(tab) {
    ...
    ({ agentes: viewAgentes, empresas: viewEmpresas, competencias: viewCompetencias, agenda: viewAgenda, saidas: viewSaidas })[tab]();
  }

  // ── View: Agentes ──
  function viewAgentes() {
    $("view").innerHTML =
      `<div class="section-title">Agentes de IA</div><div class="grid-cards">` +
      AGENTES.map(
        (a) => `<div class="agent-card">
          <div class="agent-num">AGENTE ${a.n}</div>
          <h3>${esc(a.nome)}</h3><p>${esc(a.desc)}</p>
          <button class="btn btn-primary btn-sm" data-ag="${a.id}">Abrir</button></div>`,
      ).join("") +
      `</div>`;
    $("view").querySelectorAll("[data-ag]").forEach((b) => b.addEventListener("click", () => abrirAgente(b.dataset.ag)));
  }
  ...
  async function abrirAgente(id) {
    ...
    openModal(ag.nome, `${campos}<div id="ag-out"></div>`, async () => {
      ...
      try {
        ...
        const texto = r.texto ?? r.conteudo ?? "";
        out.innerHTML = renderOutput(texto);
        toast("Saída gerada e salva no histórico.");
      } catch (err) {
        out.innerHTML = `<div class="output">${esc(err.message)}</div>`;
      }
      return false; // mantém o modal aberto pra ler/copiar
    }, "Gerar");
    // auto-preenche regime ao escolher empresa (calendário)
  ...
  async function executarAgente(id) {
    ...
    if (id === "fisco") return api("/agentes/fisco", { method: "POST", body: { empresaId, intimacao: $("f-intimacao").value, observacoes: $("f-obs").value || undefined } });
  }

  function renderOutput(texto) {
    const DISC = "Conteúdo gerado por IA — a conferência pelo contador responsável é obrigatória.";
    const corpo = texto.replace(DISC, "").trimEnd();
    return `<div class="output">${esc(corpo)}<span class="disclaimer">⚠ ${DISC}</span>
      <div class="row-actions" style="margin-top:12px">
        <button class="btn btn-sm" id="out-copy">Copiar</button>
        <button class="btn btn-sm" id="out-dl">Baixar .txt</button>
      </div></div>`;
  }

  // delegação dos botões copiar/baixar (output fica dentro do modal)
  document.addEventListener("click", (e) => {
    const out = e.target.closest("#ag-out .output");
    if (!out) return;
    const texto = out.innerText.replace(/^Copiar|Baixar \.txt$/gm, "").trim();
    if (e.target.id === "out-copy") { navigator.clipboard?.writeText(texto); toast("Copiado."); }
    if (e.target.id === "out-dl") {
  ...
  function regimeLabel(r) { return (REGIMES.find((x) => x[0] === r) || [, r])[1]; }

  async function viewEmpresas() {
    $("view").innerHTML = `<div class="toolbar"><div class="section-title" style="margin:0">Empresas-cliente</div><span class="spacer"></span><button class="btn btn-primary btn-sm" id="nova-emp">+ Nova empresa</button></div><div id="emp-tbl"></div>`;
    $("nova-emp").addEventListener("click", modalNovaEmpresa);
    ...
    $("emp-tbl").innerHTML = emp.length
      ...
      : vazio("Nenhuma empresa ainda. Clique em “+ Nova empresa”.");
  ...
  // ── View: Competências (CRUD) ──
  async function viewCompetencias() {
    $("view").innerHTML = `<div class="toolbar"><div class="section-title" style="margin:0">Competências</div><span class="spacer"></span><button class="btn btn-primary btn-sm" id="nova-comp">+ Nova competência</button></div><div id="comp-tbl"></div>`;
    await carregarEmpresas();
    $("nova-comp").addEventListener("click", modalNovaCompetencia);
  ...
  function modalNovaCompetencia() {
    ...
    openModal("Nova competência", `
      ...
      async () => {
        ...
        toast("Competência criada.");
        viewCompetencias();
        return true;
      });
  }

  // ── View: Agenda de prazos ──
  function statusPrazo(dataFatal) {
    const hoje = new Date(); hoje.setHours(0, 0, 0, 0);
    const fatal = new Date(dataFatal + "T00:00:00");
    const dias = Math.round((fatal - hoje) / 86400000);
  ...
  async function viewAgenda() {
    await carregarEmpresas();
    const opts = empresasCache.map((e) => `<option value="${e.id}">${esc(e.razaoSocial)}</option>`).join("");
    $("view").innerHTML = `<div class="toolbar"><div class="section-title" style="margin:0">Agenda de prazos</div><span class="spacer"></span>
      <label style="width:280px"><select id="ag-emp">${opts || `<option value="">cadastre uma empresa</option>`}</select></label></div><div id="ag-tbl"></div>`;
    const sel = $("ag-emp");
    async function load() {
      ...
      $("ag-tbl").innerHTML = prazos.length
        ? tabela(["Obrigação", "Vencimento", "Situação", "Status", ""], prazos.map((p) => {
            ...
            const data = p.dataFatal.split("-").reverse().join("/");
            return [
        ...
        : vazio("Sem prazos. Rode o Agente 1 (Calendário) para esta empresa.");
      $("ag-tbl").querySelectorAll("[data-done]").forEach((b) => b.addEventListener("click", async () => {
        ...
        toast("Prazo marcado como cumprido."); load();
      }));
    }
  ...
  async function viewSaidas() {
    $("view").innerHTML = `<div class="section-title">Histórico de saídas</div><div id="sa-tbl"></div>`;
    await carregarEmpresas();
  ...
  function vazio(msg) { return `<div class="table-wrap"><div class="empty">${esc(msg)}</div></div>`; }

  // onConfirm retorna true → fecha; false → mantém aberto. Se onConfirm é null, só "Fechar".
